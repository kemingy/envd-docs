...
[`install.python_packages`](/zh/api/starlark/v0/install#python_packages) 和 [`config.jupyter`](/zh/api/starlark/v0/config#jupyter) 等函数用于注册信息。这些信息将被保存在内存的静态图中。`envd` 使用静态图在执行的 `envd` 建立环境。
